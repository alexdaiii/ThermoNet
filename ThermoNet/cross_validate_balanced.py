...
def build_model(
    ...
    model.add(
        layers.Conv3D(
            filters=conv_layer_sizes[0],
            kernel_size=(3, 3, 3),
            input_shape=(8, 8, 8, 14),
        )
    )
...
def cross_validate(
    ...
    """ """
    assert (
        len(X_direct) == len(X_inverse)
        and len(y_direct) == len(y_inverse)
        and len(X_direct) == len(y_direct)
    )
    ...
    for i in range(k):
        ...
        val_X_direct = X_direct[
            i * num_validation_samples : (i + 1) * num_validation_samples
        ]
        val_y_direct = y_direct[
            i * num_validation_samples : (i + 1) * num_validation_samples
        ]
        val_X_inverse = X_inverse[
            i * num_validation_samples : (i + 1) * num_validation_samples
        ]
        val_y_inverse = y_inverse[
            i * num_validation_samples : (i + 1) * num_validation_samples
        ]
        ...
        # checkpoint
        filepath = (
            "/ysm-gpfs/pi/gerstein/bl626/projects/thermonet/results/models/"
            + "fold"
            + str(i + 1)
            + "_best_model.hdf5"
        )
    ...
    mean_val_losses = np.mean(
        np.column_stack([x.history["val_loss"] for x in all_histories]), axis=1
    )
    mean_val_maes = np.mean(
        np.column_stack([x.history["val_mean_absolute_error"] for x in all_histories]),
        axis=1,
    )
...
def main():
    ...
    for conv_layer_size in conv_layer_sizes:
        for dense_layer_size in dense_layer_sizes:
            ...
            mean_val_losses, mean_val_maes = cross_validate(
                X_direct,
                X_inverse,
                y_direct,
                y_inverse,
            ...
            np.savetxt(
                os.path.join(
                    args.outdir,
                    "conv_"
                    + "_".join(str(x) for x in conv_layer_size)
                    + "_dense_"
                    + "_".join(str(y) for y in dense_layer_size)
                    + "_mean_val_losses.txt",
                ),
                mean_val_losses,
                fmt="%.3f",
            )
