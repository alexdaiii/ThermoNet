...
def parse_cmd():
    ...
    parser.add_argument(
        "--voxelsize", dest="voxelsize", type=int, default=1, help="Size of the voxel."
    )
...
def compute_voxels(
    ...
    if rotations is None:
        voxels, _, _ = getVoxelDescriptors(
            prot,
            center=center.flatten(),
            boxsize=[boxsize, boxsize, boxsize],
            voxelsize=voxelsize,
...
def main():
    ...
    voxels, voxel_centers = compute_voxels(
        args.mutation_site,
        args.pdb_file,
    ...
    for i, channel in enumerate(voxels):
        ...
        if len(voxel_centers) != len(values):
            ...
            for c, v in zip(voxel_centers, values):
                record = "{0: <6}{1: >5}  {2}  {3} {4}{5: >4}    {6: >8.3f}{7: >8.3f}{8: >8.3f}{9: >6.2f}{10: >6.2f}           C".format(
                    "ATOM", j, "CA", "ALA", "A", j, c[0], c[1], c[2], 1.00, v
                )
