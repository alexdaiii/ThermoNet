...
ROSETTA_BIN = (
    "/ysm-gpfs/apps/software/Rosetta/3.10/main/source/bin/relax.static.linuxgccrelease"
)
...
def vector_add(v, w):
    ...
    """
    if len(v) != len(w):
        raise ValueError("Incompatible vector lengths: %s vs %s" % (len(v), len(w)))
...
def compute_pdbqt(pdb_file, overwrite=False):
    ...
    adt_cmd = (
        "{}/prepare_receptor4.py -r {} -A bonds_hydrogens -U deleteAltB -o {}".format(
            ADT_PATH, pdb_file, dirname + "/" + prefix + ".pdbqt"
        )
    )
...
def compute_voxel_features(
    mutation_site, pdb_file, verbose=False, boxsize=16, voxelsize=1, rotations=None
):
    ...
    if rotations is None:
        features, _, _ = getVoxelDescriptors(
            prot,
        ...
        voxel_centers = getCenters(
            prot,
            boxsize=[boxsize, boxsize, boxsize],
            center=center.flatten(),
            voxelsize=voxelsize,
        )
        rotated_voxel_centers = rotateCoordinates(
            voxel_centers[0], rotations, center.flatten()
        )
        features, _ = getVoxelDescriptors(
            prot, usercenters=rotated_voxel_centers, validitychecks=False
        )
    # return features
...
def build_mt_struct(pdb_wt, chain_id, pos, wt, mt, overwrite=False, outdir=None):
    ...
    rosetta_cmd = (
        "{} -in:file:s {} -relax:constrain_relax_to_start_coords "
