...
def main():
    ...
    parser.add_argument(
        "-c", "--chains", dest="chains", help="chains to extract sequence for"
    )
    parser.add_argument("-o", "--output", dest="output", help="output fasta file")
    parser.add_argument(
        "-r", "--resseq", dest="resseq", help="residue seq number in the PDB file"
    )
    parser.add_argument(
        "-i",
        "--interactive",
    ...
    for c in args.chains:
        ...
        if len(seqres_sequences) == 0:
            ...
            alignment = pairwise2.align.globalms(
                coord_sequence, record.seq, 1, -0.5, -10, 0
            )
        ...
        sequences.append(
            SeqRecord.SeqRecord(
                seq=sequence, id=pdb_id.upper() + ":" + c, description=""
            )
        )
